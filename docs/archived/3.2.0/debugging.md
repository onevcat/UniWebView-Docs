...
For more information on `UniWebViewLogger`, please refer to its [documentation]((/archived/3.2.0/api/uniwebviewlogger.html).
...
On Android, you need to call the [`SetWindowUserResizeEnabled`](/archived/3.2.0/api/uniwebview.html?id=setwindowuserresizeenabled) method first. It will allow Chrome to discover and connect to the web view on Android. Follow the [Remote Debugging WebViews](https://developers.google.com/web/tools/chrome-devtools/remote-debugging/webviews) topic to know how to use Chrome DevTools with UniWebView.

#### macOS

On macOS you need to call the [`SetWindowUserResizeEnabled`](/archived/3.2.0/api/uniwebview.html?id=setwindowuserresizeenabled) method first. Then you are able to right click on the web view in macOS editor to bring up the context menu. Select the "Inspect Element" in that menu.
