...
If the current platform or graphics backend supports neither the optimized stream nor a CPU readback fallback,
`Texture` remains `null` and `IsReady` remains `false`. The stream API does not fall back to the legacy PNG-backed
texture APIs. On Android, the optimized native texture path requires OpenGL ES 2 or OpenGL ES 3; under other graphics
APIs such as Vulkan, the stream automatically uses a CPU readback fallback with the same behavior at a higher per-frame
cost.
